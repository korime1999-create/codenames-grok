...
st.markdown("**Вставь все слова с доски** (через запятую или с новой строки):")

words_input = st.text_area("", height=200, placeholder="врач, стоматолог, халат, сердце, пистолет, гном, медведь...")
...
if st.button("🔍 Проанализировать и предложить шифры", type="primary", use_container_width=True):
    ...
    if words_input:
        ...
        for cat_key, cat_name in CATEGORY_NAMES.items():
            ...
            if found:
                ...
                emoji = {"person": "👤", "mythical_being": "🧌", "clothing": "👕", "furniture": "🪑",
                         "organ": "🫀", "weapon": "⚔️", "animal": "🐺", "feeling_quality": "❤️"}.get(cat_key, "•")
        ...
        if "person" in groups and len(groups["person"]) >= 2:
            n = len(groups["person"])
            suggestions.append(f"**медик / профессия** — на {n} → {', '.join(groups['person'])}")
        ...
        if "mythical_being" in groups and len(groups["mythical_being"]) >= 2:
            suggestions.append(f"**сказка** — на {len(groups['mythical_being'])} → {', '.join(groups['mythical_being'])}")
